...
namespace NYB.DeviceManagementSystem.BLL
{
    ...
    {
        public CResult<List<ProjectViewModel>> GetProjectList(out int totalCount, string userID, string searchInfo, int pageIndex = 1, int pageSize = 10, string orderby = null, bool acending = false)
        {
            using (DeviceMgmtEntities context = new DeviceMgmtEntities())
            {
                ...
                return new CResult<List<ProjectViewModel>>(result);
            }
        }
    }
}
